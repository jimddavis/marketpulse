...
- **Descriptive variable names.** Name a variable for what it holds (`station`, `member`, `column`, `tarball`), never a bare single letter (`d`, `m`, `c`, `tf`) — *including* loop and comprehension variables. Exceptions: established project aliases (`F` = `pyspark.sql.functions`, `df` / `*_df` for DataFrames), a throwaway `_`, and `e` in exception handlers (`except … as e:`) and exception-typed parameters (`e: BaseException`) — this is the exact form this guide's own error-handling examples use (§10.1, §11.4: `except Exception as e: step.fail(e); raise`), so renaming it would contradict them for no readability gain. A cryptic name saves one keystroke at write time and costs a lookup on every read.
